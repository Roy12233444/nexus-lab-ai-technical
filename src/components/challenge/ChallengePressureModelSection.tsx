...
const DIMENSION_ICONS: Record<PressureDimension, LucideIcon> = {
  TIME: Clock,
  UNCERTAINTY: Compass,
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {dimensions.map((dim) => {
          const IconComponent = DIMENSION_ICONS[dim.dimension];
              ...
              <div className="border-border/60 mt-6 border-t pt-4">
                ...
                <div className="mt-1.5 flex flex-wrap gap-1.5">
                  {dim.governingChallenges.map((cid) => (
                    <span
                      key={cid}
                      className="border-border bg-background text-foreground inline-flex items-center rounded-md border px-2 py-0.5 font-mono text-xs font-semibold"
                    >
                      {cid}
                    </span>
                  ))}
